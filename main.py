...
def explode_potatoes(grid, multipliers):
    exploded = False
    removed = []
    potatoes = [(r, c) for r in range(GRID_SIZE) for c in range(GRID_SIZE) if grid[r][c] == "POTATO"]
    ...
    for r, c in potatoes:
        ...
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                nr, nc = r + dr, c + dc
                if 0 <= nr < GRID_SIZE and 0 <= nc < GRID_SIZE:
                    if grid[nr][nc] not in ["HOT", "POTATO"]: removed.append((nr, nc))
...
def run_single_spin(bet_amount, force_hot=False, current_multipliers=None):
    ...
    multipliers = [row[:] for row in current_multipliers] if current_multipliers else [[1]*GRID_SIZE for _ in range(GRID_SIZE)]
    spin_history = []
    
    # 傳送初始畫面給前端，讓玩家能看見剛落下的土豆
    spin_history.append({
        "grid": [row[:] for row in grid], "step_win": 0, "multipliers": [row[:] for row in multipliers], "action": "initial"
    })
    ...
    while cascading:
        ...
        if clusters:
            ...
            for cluster in clusters:
                ...
                cluster_mult_sum = sum([multipliers[r][c] for r, c in cluster["positions"] if multipliers[r][c] > 1])
                final_mult = cluster_mult_sum if cluster_mult_sum > 0 else 1
                step_win += bet_amount * base_mult * final_mult
                removed.extend(cluster["positions"])
                
            for r, c in removed: grid[r][c] = "EMPTY"
            total_win += step_win
            spin_history.append({"grid": [row[:] for row in grid], "step_win": step_win, "multipliers": [row[:] for row in multipliers], "action": "clear"})
            grid = apply_avalanche(grid)
            spin_history.append({"grid": [row[:] for row in grid], "step_win": 0, "multipliers": [row[:] for row in multipliers], "action": "drop"})
            continue
            
        if explode_potatoes(grid, multipliers):
            spin_history.append({"grid": [row[:] for row in grid], "step_win": 0, "multipliers": [row[:] for row in multipliers], "action": "explode"})
            grid = apply_avalanche(grid)
            spin_history.append({"grid": [row[:] for row in grid], "step_win": 0, "multipliers": [row[:] for row in multipliers], "action": "drop"})
            continue
...
async def spin(request: SpinRequest):
    ...
    if hot_count >= 3:
        history.append({"is_free_spin_trigger": True, "grid": history[-1]["grid"], "step_win": 0, "multipliers": final_mults, "action": "none"})
